...
#################################
#################################
filename = 'RvsH_tempscan_5T.dat'#raw data from PPMS
direction = -1 #-1 takes +B->-B, +1 takes -B->+B
Bmax = 5.001 #max field used; important for interpolation fucntion
...
config = 3 #set to 1, 2 or 3 to choose what channels are being read for the data (see below)
...
for i in range(len(split_data_field)):
    ...
    Bm = Bp*-1
    Rxxint = interp1d(Bp, Rxx)#sets up interpolation function
    Ryyint = interp1d(Bp, Ryy)
    Rxyint = interp1d(Bp, Rxy)
    ...
    ##############derivative##################
    Rxysymint = interp1d(Bp, Rxysym)#sets up interpolation function
    ...
    dRxydBintoutput = interp1d(Bint, dRxydB)#interpolates results to values of Bp
    dRxydBoutput= dRxydBintoutput(Bp)

    Rsqrsymint = interp1d(Bp, Rsqr)#sets up interpolation function
    dRsqrdB=np.gradient(Rsqrsymint(Bint))*len(Bint)/2*1/Bmax#calculates derivative
    #note that the this is dRxy/dB = dB'/dB*dRxy/dB'; where B' = len(Rxy)/(2Bmax)*B+len(Rxy)/2
    dRsqrdBintoutput = interp1d(Bint, dRsqrdB)#interpolates results to values of Bp
