...
#################################
#################################
filename = 'RvsH_2K_9T.dat'#raw data from PPMS
...
for i in range(len(split_data)):   
    ...
    split_data_field[i] = np.split(datacenter,[changeB[0][1],changeB[0][2]])[1]# rename to upsweep later
...
##Combines up and down symmetrized scans.  B will be in the form of Bup + Bupm
    ...
    plt.figure(4)
    plt.plot(B, Rxxsym, marker='.')
    plt.figure(5)
    plt.plot(B, Ryysym, marker='.')
    plt.figure(6)
    plt.plot(B, Rxysym, marker='.')
