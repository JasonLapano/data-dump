...
def import_data(filename,
    ...
    direction  #-1 takes +B->-B, +1 takes -B->+B
    Bmax = 14.01 #max field used; important for interpolation fucntion
...
##############################################################################################################
    ...
    if mag == 'FM':
        ...
        print('default is PM, add mag="FM" for ferromagnetic')
        
    if plot=='Yes':
        
        transport.plot(final_data, save=save)
...
def plot(s, filename, save='Yes'):
    
    templist = zeros(len(s))
    for i in range(len(s)):
        templist[i] = np.round(s[i][0, 0])
