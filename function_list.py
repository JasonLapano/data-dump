# -*- coding: utf-8 -*-
"""
Created on Fri May  7 13:25:15 2021
...
def bin_fun(xvals,binnum):
    loopnumber=round(len(xvals)/binnum)
    temp=np.empty([loopnumber-1,len(xvals[0,:])])
    i=0
    for i in range(0,loopnumber-1):
        aves=np.zeros([1,len(xvals[0,:])])
        j=0
        for j in range(-binnum,binnum):
            aves=aves+xvals[j+i*binnum,:]
        temp[i,:]=aves[0,:]/(2*binnum)
    return temp


def moving_ave(xvals,binnum):
    newlen=len(xvals[:,0])-2*binnum
    temp=np.empty([newlen,len(xvals[0,:])])
    i=0
    for i in range(0,newlen):
        aves=np.zeros([1,len(xvals[0,:])])
        j=0
        for j in range(-binnum,binnum):
            aves=aves+xvals[j+binnum+i,:]
        temp[i,:]=aves[0,:]/(2*binnum)
    return temp
