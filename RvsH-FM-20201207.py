...
saturation = 10 # % of data used to subtract the linear background
#################################
#################################
#################################
#################################
...
##############ALL THIS STUFF NEEDS REDONE################################################
    ...
    final_data[i]=np.stack((Tadj,B,Rxysym,Rxy_sub, rho,dRxydBoutput,dRsqrdBoutput,drhoBoutput,Rsqr,Rsym,Rxy,Rxx,Ryy),axis=1)
    ...
    ax5 = plt.subplot(435)
    plt.plot(Bup[np.where(np.absolute(Bup)<2)], Rsym_up[np.where(np.absolute(Bup)<2)]*geo)
    plt.plot(Bup[np.where(np.absolute(Bup)<2)], Rsym_down[np.where(np.absolute(Bup)<2)]*geo)
    ax5.set_title("R Symmeterized")
    ax5.set_xlabel("Field(B)")
    ax5.set_ylabel("Resistance(Ohm)")
    ax5.set_xlim([-2, 2])
    plt.show
    ...
    ax6 = plt.subplot(436)
    plt.plot(Bup, ((Rsym_up-R0)/R0)*100*geo)
    plt.plot(Bup, ((Rsym_down-R0)/R0)*100*geo)
    ...
    figname=filename.strip('.dat')+'_{:}K.csv'.format(templist[i])
...
head = 'T(K),B(T),Rxy(ohms),Rxy-background(ohm), rho(ohm*cm),dRxy/dB,dRsqr/dB,drho/dB,Rave(ohm/sq),Rsym(ohm),Rxy_raw,Rxx_raw,Ryy_raw,'
