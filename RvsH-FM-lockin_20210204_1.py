...
#################################
#################################
filename = '20201021_1_RvsH_2K_90000T.dat'#raw data from PPMS
...
#                      usecols=(3,4,21,21,22,0), # Only read these columns
#                      comments='Measurement') # Ignore bad lines
...
#                      delimiter=',', # How is the data delimited?
#                      skip_header=32, # Skip the first 32 rows
#                      usecols=(3,4,19,20,21,0), # Only read these columns
#                      comments='Measurement') # Ignore bad lines
...
for i in range(len(upsweep)):
    ...
    Rxxint_up = interp1d(Bup, Rxx_up)#sets up interpolation functions for upsweep
    Ryyint_up = interp1d(Bup, Ryy_up)
    Rxyint_up = interp1d(Bup, Rxy_up)
    
    Rxxint_down = interp1d(Bdown, Rxx_down_pre)#sets up interpolation function for downsweep
    Ryyint_down = interp1d(Bdown, Ryy_down_pre)
    Rxyint_down = interp1d(Bdown, Rxy_down_pre)
...
##############ALL THIS STUFF NEEDS REDONE################################################
    ...
    ##############derivative##################
    Rxysymint_up = interp1d(Bup, Rxysym_up)#sets up interpolation function
    ...
    dRxydBintoutput_up = interp1d(Bint_up, dRxydB_up)#interpolates results to values of Bp
    dRxydBoutput_up= dRxydBintoutput_up(Bup)

    Rsqrsymint_up = interp1d(Bup, (Rxxsym_up/2+Ryysym_up/2)*geo)#sets up interpolation function
    dRsqrdB_up=np.gradient(Rsqrsymint_up(Bint_up))*len(Bint_up)/2*1/Bmax#calculates derivative
    #note that the this is dRxy/dB = dB'/dB*dRxy/dB'; where B' = len(Rxy)/(2Bmax)*B+len(Rxy)/2
    dRsqrdBintoutput_up = interp1d(Bint_up, dRsqrdB_up)#interpolates results to values of Bp
    dRsqrdBoutput_up= dRsqrdBintoutput_up(Bup)
    drhoBoutput_up=thickness*dRsqrdBoutput_up
    
    ############Derivative for down slope########
    Rxysymint_down = interp1d(Bup, Rxysym_down)#sets up interpolation function
    ...
    #note that the this is dRxy/dB = dB'/dB*dRxy/dB'; where B' = len(Rxy)/(2Bmax)*B+len(Rxy)/2
    dRxydBintoutput_down = interp1d(Bint_down, dRxydB_down)#interpolates results to values of Bp
    dRxydBoutput_down= dRxydBintoutput_down(Bup)

    Rsqrsymint_down = interp1d(Bup, (Rxxsym_down/2+Ryysym_down/2)*geo)#sets up interpolation function
    dRsqrdB_down=np.gradient(Rsqrsymint_down(Bint_down))*len(Bint_down)/2*1/Bmax#calculates derivative
    #note that the this is dRxy/dB = dB'/dB*dRxy/dB'; where B' = len(Rxy)/(2Bmax)*B+len(Rxy)/2
    dRsqrdBintoutput_down = interp1d(Bint_down, dRsqrdB_down)#interpolates results to values of Bp
    ...
    final_data[i]=np.stack((Tadj,B,inv_B, Rxysym,Rxy_sub, rho, mag_res,dRxydBoutput,dRsqrdBoutput,drhoBoutput,Rsqr,Rsym,Rxy,Rxx,Ryy, fRxx_diff, fRxy_diff),axis=1)
...
head = 'T(K),B(T),1/B, Rxy(ohms),Rxy-background(ohm), rho(ohm*cm), Magneto,dRxy/dB,dRsqr/dB,drho/dB,Rave(ohm/sq),Rsym(ohm),Rxy_raw,Rxx_raw,Ryy_raw, Delta-Rxx, Delta-Rxy'
